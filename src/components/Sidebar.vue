...
<nav class="panel is-primary sidebar">
    ...
    <a v-if="availableUpdate.version && !availableUpdate.updating" class="panel-block has-text-info has-tooltip-info has-tooltip-right" 
        @click="emit('update')"  data-tooltip="Click to install update"
    >
        <Icon icon="download-circle">Update to v{{ availableUpdate.version }}</Icon> 
    </a>
    <span v-else-if="availableUpdate.version && availableUpdate.updating" class="panel-block has-text-info">
        <Icon icon="download">Downloading update...</Icon> 
    </span>
...
import { AddonCounts, StaticAppData, UpdateData } from '../types/App.ts';
...
const emit = defineEmits(["scan", "check-update", "update"])
