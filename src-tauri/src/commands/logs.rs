...
pub async fn open_logs_folder(app: AppHandle) -> Result<(), String> {
    ...
    app.opener().open_path(logs_path.to_string_lossy().to_string(), None::<&str>).map_err(|e| e.to_string())
}
...
time: {}
----------------------------------------------------
"#, env!("CARGO_PKG_NAME"), env!("CARGO_PKG_VERSION"), chrono::Local::now().format("%Y-%m-%d %H:%M:%S"));
    content.insert_str(0, &prefix);

    debug!("uploading logs.... ({})", content.len());
    Notification::new(NotificationType::Info, "Uploading logs...".to_string(), None).send(&app);
    let result = client.post(format!("https://paste.jackz.me/paste?expires={}", UPLOAD_LOGS_EXPIRES.as_secs()))
    ...
    debug!("log id={} url={} delete_token={}", result.name, result.url, result.delete_token);
    Notification::new(NotificationType::Info, "Logs uploaded".to_string(), Some(format!("URL: {}", result.url))).send(&app);
    app.opener().open_path(result.url, None::<&str>).map_err(|e| e.to_string())
}
